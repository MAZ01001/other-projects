...
namespace ConsoleIO{
    ...
    public static class Run{
        ...
        ///     <br/><seealso cref="ScrollAnim"/>
        /// </summary>
        /// <param name="dir">[Optional] The direction in wich to scroll - defaults to up</param>
        ...
        /// <param name="freq">[Optional] the frequancy of the tone in Hertz - [37 to 32767] - defaults to 800</param>
        ...
        /// <exception cref="freq">if <paramref name="freq"/> is not in range [37 to 32767]</exception>
        ...
        public static void PlayTone(Int32 freq = 800, Int32 time = 200, bool wait = true){
            if(freq is < 37 or > 32767) throw new ArgumentOutOfRangeException(nameof(freq), "must be in range [37 to 32767]");
        ...
        public static string GetInput(ushort MaxInputSize=10000){
            int _c=CursorX,_r=CursorY,_i=0;
            string _s="";
            bool _end=false;
        ...
        /// <summary>[private] imported function to set window size (maximize/minimize/restore/hidden/...)<br/>used for <see cref="SetFullscreen"/> and <see cref="RevertFullscreen"/></summary>
        ...
        /// <summary>[private] imported function to get the console window process handle<br/>used for <see cref="ShowWindow"/>'s first parameter</summary>
        ...
        /// <summary>
        ///     get or set visibility of the console cursor<br/>
        ///     see also:<br/>
        ///     <seealso cref="CursorSize"/>
        /// </summary>
        ...
        /// <summary>
        ///     get or set the console background color<br/>
        ///     see also:<br/>
        ///     <seealso cref="ForegroundColor"/><br/>
        ...
        /// <summary>
        ///     get or set the console foreground color<br/>
        ///     see also:<br/>
        ///     <seealso cref="BackgroundColor"/><br/>
        ///     <seealso cref="Colors"/><br/>
        ///     <seealso cref="SetColor"/><br/>
        ///     <seealso cref="ResetColor"/>
        /// </summary>
        ...
        ///     <seealso cref="ResetColor"/>
        /// </summary>
        /// <param name="foregroundColor">foreground color - default [<see cref="Colors.Green"/>]</param>
        /// <param name="backgroundColor">background color - default [<see cref="Colors.Black"/>]</param>
        ...
        /// <summary>
        ///     draws the border in the console with custom chars<br/>
        ///     custom left/top/bottom/right-bars and corners in that order<br/>
        /// </summary>
        /// <param name="left">char for the left (vertical) bar of the console excluding the corners</param>
        /// <param name="top">char for the top (horizontal) bar of the console excluding the corners</param>
        /// <param name="bottom">char for the bottom (horizontal) bar of the console excluding the corners</param>
        ...
        /// <param name="col">column index (left) where to start printing - [1 to <see cref="InnerWidth"/>] - default [0] → use current</param>
        /// <param name="row">row index (top) where to start printing - [1 to <see cref="InnerHeight"/>] - default [0] → use current</param>
        ...
        public static bool Anim(string outpt,int ms=20,char interrupt=' ',bool scrolling=true,int col=0,int row=0){
            ...
            foreach(char key in outpt) switch(key){
                ...
                break;
                case '\r':SetCursorPos(1,CursorY);break;
                case '\t':
        ...
        /// <param name="interrupt">if this character is pressed during printing sets <see cref="ms"/> to 0 - default [' ']</param>
        /// <param name="scrolling">if it should scroll up if the text is to long for screen height - default [true]</param>
        /// <param name="col">column index (left) where to start printing - [1-<see cref="InnerWidth"/>] - default [0] → use current</param>
        /// <param name="row">row index (top) where to start printing - [1-<see cref="InnerHeight"/>] - default [0] → use current</param>
        /// <return>true if printing was interrupted by pressing the <see cref="interrupt"/> key or <see cref="ms"/> is 0</return>
        /// <exception cref="ArgumentOutOfRangeException">if <paramref name="ms"/> is not a positive integer</exception>
        /// <exception cref="ArgumentOutOfRangeException">if <paramref name="col"/> is not in range [1-<see cref="InnerWidth"/>]</exception>
        /// <exception cref="ArgumentOutOfRangeException">if <paramref name="row"/> is not in range [1-<see cref="InnerHeight"/>]</exception>
        public static bool AnimHor(string outpt,int ms=20,char interrupt=' ',bool scrolling=true,int col=0,int row=0){
            if(ms<0){throw new ArgumentOutOfRangeException(nameof(ms),"must be a positive integer");}
            if(col==0){col=CursorX;}
            if(row==0){row=CursorY;}
            if(col<1||col>InnerWidth){throw new ArgumentOutOfRangeException(nameof(col),"must be in range [1-innerWidth]");}
            if(row<1||row>InnerHeight){throw new ArgumentOutOfRangeException(nameof(row),"must be in range [1-innerHeight]");}
            SetCursorPos(col,row);
            ClearInput();
            foreach(char key in outpt) switch(key){
                case '\n':
                ...
                break;
                case '\r': SetCursorPos(CursorX,1); break;
                case '\t':
        ...
        /// </summary>
        /// <param name="col">column index (top) of line to clear - [1-<see cref="InnerWidth"/>]</param>
        ...
        ///     <seealso cref="ClearConsoleAll"/>
        /// </summary>
        /// <param name="row">row index (left) of line to clear - [1-<see cref="InnerHeight"/>]</param>
        /// <param name="ch">char to clear the line with</param>
        /// <exception cref="ArgumentOutOfRangeException">if <paramref name="row"/> is not in range [1-<see cref="InnerHeight"/>]</exception>
        /// <exception cref="ArgumentException">if <paramref name="ch"/> is '\r' or '\n'</exception>
    ...
    public class TestGame{
        ...
        protected static void CursorGame(int times=3){
            ...
            for(int i=0;i<times;i++){
                ...
                Run.SetColor();Run.CursorSize = 100;Run.CursorShow = true;
                do{
            ...
            Run.SetColor();Run.CursorSize=25;Run.CursorShow = false;
        }
        /// <summary>print final code on screen</summary>
        protected static void PrintFinalCode(){
            int _c=(Run.InnerWidth>>1)-28,_r=(Run.InnerHeight>>1)-6;
        ...
        protected static byte GetSingleNumStrict(){
            ...
            Run.CursorSize=100;Run.CursorShow = true;
            do{
                try{_num=Run.GetSingleNum(true);}
                catch(FormatException){_num=10;}
            }while(_num==10);
            Run.CursorSize=25;Run.CursorShow = false;
        ...
        protected static void GetPlayerName(){
            LastSkiped=false;
            do{
                Run.SetColor();Run.CursorShow = false;
        ...
        public static UInt32 GetSaveUInteger32(){
            ...
            do{
                try{
                    Run.SetColor(Run.Colors.Yellow);Run.CursorShow = true;
                    _tmp=Run.GetIntNum();
                    Run.SetColor();Run.CursorShow = false;
                    if(_tmp<0){Run.Anim("\r\nnegative numbers aren't allowed, try again: ",1);}
                }
                catch(FormatException){Run.CursorShow = false;Run.Anim("\r\nonly whole numbers may be entered, try again: ",1);}
                catch(OverflowException){Run.CursorShow = false;Run.Anim("\r\nthe number is to long, try again: ",1);}
            }while(_tmp<0);
            return (UInt32)_tmp;
        }
        ...
        public static float GetSaveUFloat(){
            ...
            do{
                ...
                catch(FormatException){Run.SetColor();Run.CursorShow = false;Run.Anim("\r\nonly numbers may be entered, try again: ",1);}
                catch(OverflowException){Run.SetColor();Run.CursorShow = false;Run.Anim("\r\nthe number is to long, try again: ",1);}
            }while(_tmp<0F);
            return _tmp;
        }
        /// <summary>prints the logo on screen with an animation</summary>
        protected static void PrintLogoAnim(){
            int _c=(Run.InnerWidth>>1)-39,_r=(Run.InnerHeight>>1)-4;
        ...
        protected static void ClearBar(int width=3,Run.ScrollDirection dir=Run.ScrollDirection.down,int time=3,char ch='█'){
            switch(dir){
                case Run.ScrollDirection.down:
                    ...
                    for(int i=1;i<=Run.InnerHeight;i++){if(i<Run.InnerHeight-width){Run.ClearRow(i+width,ch);}Run.ClearRow(i,' ');Run.Wait(time);}
                    break;
                case Run.ScrollDirection.up:
                    for(int i=0;i<width;i++){Run.ClearRow(Run.InnerHeight-i,ch);Run.Wait(time);}
                    for(int i=Run.InnerHeight;i>0;i--){if(i>width){Run.ClearRow(i-width,ch);}Run.ClearRow(i,' ');Run.Wait(time);}
                    break;
                case Run.ScrollDirection.right:
                    for(int i=1;i<=width;i++){Run.ClearCol(i,ch);Run.Wait(time);}
                    for(int i=1;i<=Run.InnerWidth;i++){if(i<Run.InnerWidth-width){Run.ClearCol(i+width,ch);}Run.ClearCol(i,' ');Run.Wait(time);}
                    break;
                case Run.ScrollDirection.left:
                    for(int i=0;i<width;i++){Run.ClearCol(Run.InnerWidth-i,ch);Run.Wait(time);}
                    for(int i=Run.InnerWidth;i>0;i--){if(i>width){Run.ClearCol(i-width,ch);}Run.ClearCol(i,' ');Run.Wait(time);}
                    break;
            }
        }
    }
